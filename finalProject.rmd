...
title: "\\vspace{-2cm} Stat 410 Aidan's Movies"
author: "Aidan Gerber"
...
The data was collected over time by me. A partial dataset in json form is available on my [website](https://www.tradethisandthat.com/movies/api/all_movies/) and a full version is available on the [project website](https://stat.aidang.me/continuous_movies.csv). 
Data from TMDB was collected using their API. Data from IMDB was collected by web scraping their pages for awards and rating distributions. My rating and metacritic ratings are collected by me. This data is relevant since my ratings would be related to ratings from critics as well as attributes of movies like genres.
...
```{r pairs, echo=FALSE, eval=SHOW_BACK_TO_TOP}
...
```{r conditional_block_glmnet3-1, echo=FALSE, results='asis', eval=SHOW_BACK_TO_TOP}
cat('### Comparison')
```
...
```{r conditional_block_glmnet4-1, echo=FALSE, results='asis', eval=SHOW_BACK_TO_TOP}
cat('### Comparison Full')
```
...
most_reviewed = data.frame(imdb_rating=c(8.8,8.3,8.6,8.6,7.9,8.4,7.9,1.9,5),imdb_count=c(1794349,1389883,1381370,1342619,1130852,1014965,350258,38184,5695),is_john_williams=c(0,0,0,1,0,0,1,0,0))
predictions = predict(fit_bic, most_reviewed, interval="prediction")
kable(cbind(Movie=c(' The Lord of the Rings 2001','Inglorious Basterds','The Silence of the Lambs', 'Saving Private Ryan', 'Titanic',  'Braveheart', 'Boyhood','The Hottie & the Nottie', 'The VelociPastor'),most_reviewed,predictions))
```
